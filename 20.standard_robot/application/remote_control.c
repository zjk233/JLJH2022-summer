...
void rc_task(void const * argument)  //遥控器数据发送任务（若不使用可关闭）
{
     
	
    while(1)
    {
			rc_info_.ch[0]=rc_ctrl.rc.ch[0];
			rc_info_.ch[1]=rc_ctrl.rc.ch[1];
			rc_info_.ch[2]=rc_ctrl.rc.ch[2];
			rc_info_.ch[3]=rc_ctrl.rc.ch[3];
			rc_info_.ch[4]=rc_ctrl.rc.ch[4];			
			rc_info_.s[0]=rc_ctrl.rc.s[0];
			rc_info_.s[1]=rc_ctrl.rc.s[1];
			
			rm_queue_data(RC_ID,&rc_info_,sizeof(rc_info_t));
